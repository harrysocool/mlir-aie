...
#
# Batched (grouped) matmul derived from whole_array.py.
# Computes G independent matmuls C[g] = A[g] @ B[g] in ONE NPU dispatch.
# A and C are stacked linearly in the M dimension ((G*M, K) and (G*M, N)),
# so the existing per-row offsets work unchanged; only B carries a per-group
# offset: B_batch_offset = g * K * N, g = (row_base+tile_row)//rows_per_head,
# rows_per_head = (M//G)//m//n_aie_rows. The B runtime operand is (G*K*N,).
# Set the group count via the BMM_G environment variable (default 1, i.e.
# identical to whole_array.py). Pass M = G * M_orig on the command line.
import argparse
import numpy as np
import os
BMM_G = int(os.environ.get('BMM_G','1'))
...
def my_matmul(
    ...
    def device_body():
        ...
        # batched-matmul: each head is a separate K-slice of B
        M_orig = M // BMM_G
        rows_per_head = M_orig // m // n_aie_rows
        @runtime_sequence(
            np.ndarray[(M * K,), np.dtype[dtype_in]],
            np.ndarray[(BMM_G * K * N,), np.dtype[dtype_in]],
                ...
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        for tile_row in range(tb_n_rows):
                            ...
                            #     |0011    0011    |
                            #      ----------------
                            g_head = (row_base + tile_row) // rows_per_head
                            B_batch_offset = g_head * K * N
                            B_col_offset = (col * n if not b_col_maj else col * n * K) + B_batch_offset
                            if not b_col_maj:
